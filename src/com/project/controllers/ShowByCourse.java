...
@WebServlet("/ShowByCourse")
public class ShowByCourse extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			PreparedStatement ps=conn.prepareStatement("select * from course_detail where course like '%"+s1+"%'");
			...
			PreparedStatement ps3=conn.prepareStatement("select * from result_table where course like '%"+s1+"%'");
			...
			PreparedStatement ps4=conn.prepareStatement("select * from syllabus_detail where course like '%"+s1+"%'");
			...
			PreparedStatement ps5=conn.prepareStatement("select * from time_table where course like '%"+s1+"%'");
			...
			PreparedStatement ps6=conn.prepareStatement("select * from study_table where course like '%"+s1+"%'");
			...
			out.println("Previous Year Papers Found");
			
			PreparedStatement ps10=conn.prepareStatement("select * from previous_table where course like '%"+s1+"%'");
			...
			out.println("<th>FILE DETAIL</th>");
			out.println("<th>FILE</th>");
			out.println("</tr>");
			
			while(rs10.next())
			{
				String id1=rs10.getString(1);
				
				out.println("<tr>");
				out.println("<td>"+rs10.getString(1)+"</td>");
				out.println("<td>"+rs10.getString(2)+"</td>");
				out.println("<td>"+rs10.getString(3)+"</td>");
				out.println("<td>"+rs10.getString(4)+"</td>");
				out.println("<td>"+rs10.getString(5)+"</td>");
				out.println("<td><a href='downloadadmin2?id="+id1+"'>View File</a></td>");
				out.println("</tr>");
			}
			out.println("</table>");
